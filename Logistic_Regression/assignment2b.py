...
if __name__ == '__main__':
    ...
    working_frame  = pd.DataFrame(columns=['HndWshQual','Risk', 'Flu'])
    ...
    #working_frame = working_frame.fillna(-1)
    ...
    #working_frame = working_frame[(working_frame.T !=-1).any()]
    temp_frame = working_frame[['HndWshQual','Risk']].copy(deep=True)
    temp_frame = normlize_dataframe(temp_frame)
    working_frame[['HndWshQual', 'Risk']]= temp_frame
    sns.pointplot(x='HndWshQual', y='Risk', hue='Flu' , data=working_frame, saturation=0.8)
    plt.show()
    exit()
