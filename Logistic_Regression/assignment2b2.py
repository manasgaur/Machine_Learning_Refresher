...
if __name__ == '__main__':
    ...
    print data['Risk'].max(), data['Risk'].min()
    print data['HndWshQual'].max(), data['HndWshQual'].min()
    print data['KnowlTrans'].max(), data['KnowlTrans'].min()
    print data['Flu'].max(), data['Flu'].min()
    working_frame  = pd.DataFrame(columns=['KnowlTrans','HndWshQual','Risk', 'Flu'])
    working_frame['KnowlTrans'] = data['KnowlTrans'].copy(deep=True)
    working_frame['HndWshQual'] = data['HndWshQual'].copy(deep=True)
    ...
    working_frame = working_frame[np.isfinite(working_frame['HndWshQual'])]
    working_frame = working_frame[np.isfinite(working_frame['KnowlTrans'])]
    temp_frame = working_frame[['KnowlTrans','HndWshQual','Risk']].copy(deep=True)
    temp_frame = normlize_dataframe(temp_frame)
    working_frame[['KnowlTrans','HndWshQual', 'Risk']]= temp_frame
    analyzing_features(working_frame)
    exit()
    working_frame['KnowlTrans'] =  working_frame['KnowlTrans'].round(decimals=2)
    sns.pointplot(x='KnowlTrans', y='Risk', hue='Flu', data=working_frame)
