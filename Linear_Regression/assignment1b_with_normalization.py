'''
Trained linear regression model over a normalized dataset and calculated p-value using pearson chi-square
...
-- Did 4 plots
 ----- Cummulative total plot ( def analyzing_features(df))
 ----- Training error plot (def learning_error_plot(l_error))
 ----- Prediction error plot (def prediction_error_plot(learning_rate, model_score_lst))
 ----- Regression line plot (def regression_line_plot(feat,observed, ypred))
...
def covariance_between_feature_target(df_list):
    ...
    feature2=df_list['KnowlTransSq'].tolist()
...
''' I have used this function. You can even call normalization(df_list) function.'''
def normalize_dataframe(df):
    '''

    :param df:
    :return: Min-max normalized dataframe using pandas
    '''
...
def chi_sq_test(df, col1, col2):
    '''
    params : df (dataset), col1 (KnowlTrans), col2 (KnowlTransSq)
    results : p-value of the col1 and col2
    '''
    grouping = df.groupby([col1, col2]).size()
    result = grouping.unstack(col1)
    gtest,pvalue,a,b= chi2_contingency(result.fillna(0))
    print pvalue
...
if __name__ == '__main__':
    ...
    '''
    working_frame = pd.DataFrame(columns=['KnowlTrans','KnowlTransSq','Risk'])
    working_frame['KnowlTrans'] = data['KnowlTrans'].copy(deep=True)
    working_frame['KnowlTransSq'] = data['KnowlTrans'].copy(deep=True)
    working_frame['KnowlTransSq'] = working_frame['KnowlTransSq']**2
    ...
    working_frame = normalize_dataframe(working_frame)
    print "Calculating the pearson chi-square test to check the distributional difference between features"
    chi_sq_test(working_frame,'KnowlTrans','KnowlTransSq')
    ...
    print "covariance between (Knowltrans, KnowlTransSq) and Risk" , covariance_between_feature_target(working_frame)
